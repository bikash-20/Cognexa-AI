"""Provider fallback chain: OpenAI → Cloudflare → OpenRouter → Pollinations.
   Each provider is wrapped with timeout, circuit-breaker, and structured logging.
   When all fail, the API still returns a degraded but valid reply.
"""
...
@dataclass
class Circuit:
    """Tiny per-provider circuit breaker. Trips after 3 failures within 30 s."""
    ...
    def trip(self) -> None:
        ...
        if len(self.failures) >= 3:
            self.open_until = time.time() + 20  # cooldown
...
async def generate(system: str, user: str, *, timeout: float) -> tuple[str, str]:
    """Returns (text, provider_name). provider_name is "degraded" if all fail."""
    ...
    for name, fn in _CHAIN:
        try:
            ...
            return text, name
        except Exception as e:  # noqa: BLE001 — broad on purpose; we log + continue
            _cb(name).trip()
            last_err = e
